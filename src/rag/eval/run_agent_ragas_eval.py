...
Sibling to `run_ragas_eval.py`: that module scores the classic
`RetrievalPipeline.answer()` path; this one scores the agent graph's
actual output (`rag.agent.graph.run_agent()`, via
`run_agent_eval.evaluate_agent`) for `agentic_extension_gold.jsonl`.
RAGAS never drives routing/tool-selection/retry scoring; those stay
`run_agent_eval.py`'s deterministic metrics. This module only judges
final-answer/evidence quality for whichever route the agent took.

Each row's `contexts` comes from the evidence one
`evaluate_agent(..., include_evidence=True)` pass already gathered (not a
second agent run), so the judged context is exactly what the agent's own
synthesize step saw. Each source still passes through
`egress_policy.apply_egress_policy` before it may reach the hosted judge,
matching `run_ragas_eval.py`'s own gating.

`expected_tool_sequence`/`expected_subquestions`/`expected_reformulation`
are gold-authoring/scoring-only fields, never read by `rag.agent.*` (the
agent only ever sees `GoldExample.question` via
`AgentState.original_query`). `expected_answer`/`reference_contexts` are
read here, in the eval/judge path only, exactly like `run_ragas_eval.py`.
